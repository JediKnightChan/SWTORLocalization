...
class TorEditor:
    ...
    def extract_file(self, tor_path, ext="default", save=True):
        with open(self.dirassets + "swtor_en-us_global_1.tor", "rb") as tor_obj:
            result = self.look_for_tor_path(tor_obj, tor_path, for_extraction=True)
            required_archive_name = "swtor_en-us_global_1.tor"
        if result is None:
        ...
        if result is not None:
            with open(self.dirassets + required_archive_name, "rb") as tor:
    ...
    def change_file_in_tor(self, new_file_content, tor_path, tor_archive_name=None):
        result = None
        if tor_archive_name:
            with open(self.dirassets + tor_archive_name, "rb") as tor_archive:
        ...
        tor_archive_len = os.path.getsize(self.dirassets + tor_archive_name)
        ...
        if new_compr_size > result["compr_size"]:
            ...
            with open(self.dirassets + tor_archive_name, "ab") as tor_archive:
...
if __name__ == "__main__":
    tor = TorEditor("C:/Program Files (x86)/Electronic Arts/BioWare/Star Wars - The Old Republic/")
    #tor_paths = read_manifest()
    #tor.extract_everything(tor_paths)
    #tor.extract_file("/resources/en-us/bnk2/cnv_location_korriban_class_sith_sorcerer_spindrall.acb")

    with open("hashnames_stb.txt") as file:
        cont = file.readlines()
    tor.extract_everything(cont, format="xml")
